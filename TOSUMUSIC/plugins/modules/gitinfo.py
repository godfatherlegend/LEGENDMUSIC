...
from daxxhub import daxxhub as papatosu
...
async def daxxhub(_, message):
    ...
    papaTOSU(f"{text}").save(f"tosuhub_{message.from_user.id}.png")
    await message.reply_photo(f"hub_{message.from_user.id}.png")
    os.remove(f"tosuhub_{message.from_user.id}.png")
####

@app.on_message(filters.command(["github", "git"]))
async def github(_, message):
    if len(message.command) != 2:
        await message.reply_text("/git TOSUBOLTE")
        return
